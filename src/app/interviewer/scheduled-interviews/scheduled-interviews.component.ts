...
export class ScheduledInterviewsComponent implements OnInit {
  ...
  feedback() {
    ...
    localStorage.setItem("feedback", JSON.stringify(this.feedbackList));
    alert("You have sucessfully submitted your feedback");
  }

  getData() {
    this.candidate = this.dataService.getCandidate();
    this.interview = this.dataService.getInterviewer();
    this.schedule = this.dataService.getSchedule();
    this.feedbackList = this.dataService.getFeedback();
  }
